...
export default function ProducerTransferHistory({ limit = 25 }: { limit?: number }) {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from("movements")
      .select(
        "id,date,ring_number,transfer_reason,notes,to_producer_id," +
  ...
  return (
    <section className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <h2 style={{ margin: 0 }}>Últimas transferências</h2>
      ...
      {rows.length === 0 ? (
        <p style={{ marginBottom: 0, opacity: 0.8 }}>Sem transferências registadas.</p>
      ) : (
